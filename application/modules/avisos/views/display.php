<h2 class="header smaller lighter blue">avisos</h2>
<div class="table-header"><i class='icon-zoom-in bigger-130'></i> <a href="<?=base_url('avisos/create') ?>" style='color: #fff'>NOVO AVISO</a></div>
...
<div id="sample-table-2_wrapper" class="dataTables_wrapper" role="grid">
	...
	<table aria-describedby="sample-table-2_info" id="sample-table-2" class="table table-striped table-bordered table-hover dataTable">
		<thead>
			<tr role="row">
				<th style="width: 10%;"  class="sorting">Código</th>
				<th style="width: 75%;" class="sorting">Avisos</th>
				<th style="width: 15%;"  class="sorting_disabled">Ações</th>
			</tr>
		</thead>
		...
		<tbody aria-relevant="all" aria-live="polite" role="alert">
			 <?php 
                  foreach ($query->result() as $row) {
                      $edit_url=base_url().'avisos/create/'.$row->idavisos; 
                      $del_url=base_url().'avisos/index/'.$row->idavisos;  
                                   
                      echo" <tr class='odd'>
                        <td class='center'>$row->idavisos</td>
        
                        
                         <td class='hidden-480'>$row->aviso</td>
